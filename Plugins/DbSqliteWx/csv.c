...
# define CSV_SCHEMA   (azPValue[2])
  ...
  *ppVtab = (wx_sqlite3_vtab*)pNew;
  ...
  if( CSV_SCHEMA==0 ){
    ...
    char *zSep = "";
    int iCol = 0;
    wx_sqlite3_str_appendf(pStr, "CREATE TABLE x(");
    if( nCol<0 && bHeader<1 ){
      nCol = 0;
      do{
    ...
    if( nCol>0 && bHeader<1 ){
      for(iCol=0; iCol<nCol; iCol++){
        wx_sqlite3_str_appendf(pStr, "%sc%d TEXT", zSep, iCol);
        zSep = ",";
      }
    }else{
      do{
        ...
        if( (nCol>0 && iCol<nCol) || (nCol<0 && bHeader) ){
          wx_sqlite3_str_appendf(pStr,"%s\"%w\" TEXT", zSep, z);
          zSep = ",";
          iCol++;
        }
      }while( sRdr.cTerm==',' );
    ...
    CSV_SCHEMA = wx_sqlite3_str_finish(pStr);
  ...
  if( rc ){
    csv_errmsg(&sRdr, "bad schema: '%s' - %s", CSV_SCHEMA, wx_sqlite3_errmsg(db));
    goto csvtab_connect_error;
  }
