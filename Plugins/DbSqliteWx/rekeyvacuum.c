...
**
** Change 0: Rename function to wx_sqlite3RunVacuumForRekey()
** Change 1: Add parameter 'int nRes'
** Change 2: Remove local variable 'int nRes'
** Change 3: Remove initialization 'nRes = wx_sqlite3BtreeGetOptimalReserve(pMain)'
**
** This code is generated by the script rekeyvacuum.sh from SQLite version 3.31.1 amalgamation.
*/
SQLITE_PRIVATE SQLITE_NOINLINE int wx_sqlite3RunVacuumForRekey(
  char **pzErrMsg,        /* Write error message here */
  wx_sqlite3 *db,            /* Database connection */
  int iDb,                /* Which attached DB to vacuum */
  wx_sqlite3_value *pOut     /* Write results here, if not NULL. VACUUM INTO */
, int nRes){
  ...
  if( pOut ){
    if( wx_sqlite3_value_type(pOut)!=SQLITE_TEXT ){
      wx_sqlite3SetString(pzErrMsg, db, "non-text filename");
      return SQLITE_ERROR;
    }
    zOut = (const char*)wx_sqlite3_value_text(pOut);
  ...
  */
  ...
  if( pOut ){
    ...
    i64 sz = 0;
    if( id->pMethods!=0 && (wx_sqlite3OsFileSize(id, &sz)!=SQLITE_OK || sz>0) ){
      rc = SQLITE_ERROR;
      wx_sqlite3SetString(pzErrMsg, db, "output file already exists");
      goto end_of_vacuum;
    }
  ...
  */
  ...
  if( rc!=SQLITE_OK ) goto end_of_vacuum;
  rc = wx_sqlite3BtreeBeginTrans(pMain, pOut==0 ? 2 : 0, 0);
  if( rc!=SQLITE_OK ) goto end_of_vacuum;
  ...
  */
  {
    ...
    */
    ...
    assert( pOut!=0 || 1==wx_sqlite3BtreeIsInTrans(pMain) );
