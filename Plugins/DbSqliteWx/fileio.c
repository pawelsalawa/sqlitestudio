...
static void readFileContents(wx_sqlite3_context *ctx, const char *zName){
  ...
  mxBlob = wx_sqlite3_limit(db, SQLITE_LIMIT_LENGTH, -1);
  if( nIn>mxBlob ){
    wx_sqlite3_result_error_code(ctx, SQLITE_TOOBIG);
    fclose(in);
    return;
  }
  pBuf = wx_sqlite3_malloc64( nIn ? nIn : 1 );
  if( pBuf==0 ){
    wx_sqlite3_result_error_nomem(ctx);
    fclose(in);
    return;
  }
  if( nIn==(wx_sqlite3_int64)fread(pBuf, 1, (size_t)nIn, in) ){
    wx_sqlite3_result_blob64(ctx, pBuf, nIn, wx_sqlite3_free);
  }else{
    wx_sqlite3_result_error_code(ctx, SQLITE_IOERR);
...
static int fsdirNext(wx_sqlite3_vtab_cursor *cur){
  ...
  if( S_ISDIR(m) ){
    ...
    if( iNew>=pCur->nLvl ){
      int nNew = iNew+1;
      wx_sqlite3_int64 nByte = nNew*sizeof(FsdirLevel);
      FsdirLevel *aNew = (FsdirLevel*)wx_sqlite3_realloc64(pCur->aLvl, nByte);
...
int wx_sqlite3_fileio_init(
  ...
  rc = wx_sqlite3_create_function(db, "readfile", 1, 
                               ...
                               readfileFunc, 0, 0);
  if( rc==SQLITE_OK ){
    rc = wx_sqlite3_create_function(db, "writefile", -1,
                                 SQLITE_UTF8|SQLITE_DIRECTONLY, 0,
                                 writefileFunc, 0, 0);
  }
  if( rc==SQLITE_OK ){
