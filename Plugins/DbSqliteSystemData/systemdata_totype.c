...
*/
static int totypeAtoi64(const char *zNum, systemdata_sqlite3_int64 *pNum, int length){
  systemdata_sqlite3_uint64 u = 0;
  ...
  if( u>LARGEST_INT64 ){
    ...
    *pNum = -(systemdata_sqlite3_int64)u;
  }else{
    *pNum = (systemdata_sqlite3_int64)u;
  }
...
static int totypeAtoF(const char *z, double *pResult, int length){
  ...
  systemdata_sqlite3_int64 s = 0;       /* significand */
...
static void tointegerFunc(
  ...
  switch( systemdata_sqlite3_value_type(argv[0]) ){
    case SQLITE_FLOAT: {
      double rVal = systemdata_sqlite3_value_double(argv[0]);
      systemdata_sqlite3_int64 iVal = (systemdata_sqlite3_int64)rVal;
      if( rVal==(double)iVal ){
    ...
    case SQLITE_INTEGER: {
      systemdata_sqlite3_result_int64(context, systemdata_sqlite3_value_int64(argv[0]));
      break;
    }
    case SQLITE_BLOB: {
      const unsigned char *zBlob = systemdata_sqlite3_value_blob(argv[0]);
      if( zBlob ){
        int nBlob = systemdata_sqlite3_value_bytes(argv[0]);
        if( nBlob==sizeof(systemdata_sqlite3_int64) ){
          systemdata_sqlite3_int64 iVal;
          if( TOTYPE_BIGENDIAN ){
            int i;
            unsigned char zBlobRev[sizeof(systemdata_sqlite3_int64)];
            for(i=0; i<sizeof(systemdata_sqlite3_int64); i++){
              zBlobRev[i] = zBlob[sizeof(systemdata_sqlite3_int64)-1-i];
            }
            memcpy(&iVal, zBlobRev, sizeof(systemdata_sqlite3_int64));
          }else{
            memcpy(&iVal, zBlob, sizeof(systemdata_sqlite3_int64));
          }
          systemdata_sqlite3_result_int64(context, iVal);
        }
      }
    ...
    case SQLITE_TEXT: {
      const unsigned char *zStr = systemdata_sqlite3_value_text(argv[0]);
      if( zStr ){
        int nStr = systemdata_sqlite3_value_bytes(argv[0]);
    ...
    default: {
      assert( systemdata_sqlite3_value_type(argv[0])==SQLITE_NULL );
      break;
    }
  }
}
...
static void torealFunc(
  ...
  switch( systemdata_sqlite3_value_type(argv[0]) ){
    case SQLITE_FLOAT: {
      systemdata_sqlite3_result_double(context, systemdata_sqlite3_value_double(argv[0]));
      break;
    }
    case SQLITE_INTEGER: {
      systemdata_sqlite3_int64 iVal = systemdata_sqlite3_value_int64(argv[0]);
      double rVal = (double)iVal;
      if( iVal==(systemdata_sqlite3_int64)rVal ){
...
#endif
int systemdata_sqlite3_totype_init(
  ...
  rc = systemdata_sqlite3_create_function(db, "tointeger", 1, SQLITE_UTF8, 0,
                               tointegerFunc, 0, 0);
  if( rc==SQLITE_OK ){
    rc = systemdata_sqlite3_create_function(db, "toreal", 1, SQLITE_UTF8, 0,
                                 torealFunc, 0, 0);
  }
  return rc;
}
