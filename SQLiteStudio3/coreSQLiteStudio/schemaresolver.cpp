...
SqliteCreateTablePtr SchemaResolver::virtualTableAsRegularTable(const QString &database, const QString &table)
{
    ...
    db->exec("DROP TABLE %1;", {newTable}, Db::Flag::STRING_REPLACE_ARGS);
...
QString SchemaResolver::getObjectDdl(const QString &database, const QString &name)
{
    ...
    // Get the DDL
    QVariant results = db->exec(
                "SELECT sql FROM %1.sqlite_master WHERE lower(name) = '%2';",
                {dbName, escapeString(lowerName)},
                Db::Flag::STRING_REPLACE_ARGS
            )->getSingleCell();
...
QStringList SchemaResolver::getObjects(const QString &database, const QString &type)
{
    ...
    SqlResultsPtr results = db->exec(
                "SELECT name FROM %1.sqlite_master WHERE type = '%2';",
                {dbName, type},
                Db::Flag::STRING_REPLACE_ARGS
            );
...
QStringList SchemaResolver::getAllObjects(const QString& database)
{
    ...
    SqlResultsPtr results = db->exec(
                "SELECT name, type FROM %1.sqlite_master;",
                {dbName},
                Db::Flag::STRING_REPLACE_ARGS
            );
...
QStringList SchemaResolver::getFkReferencingTables(const QString& database, const QString& table)
{
    ...
    {
        results = db->exec("PRAGMA %1.foreign_key_list(%2)", {wrapObjIfNeeded(database, dialect), wrapObjIfNeeded(fkTable, dialect)},
                           Db::Flag::STRING_REPLACE_ARGS);
